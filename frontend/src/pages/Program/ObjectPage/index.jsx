...
import { getSingleRecord, handleSubmit, validationSchema } from '@pages/Program/api';
...
const ObjectPage = (props) => {
	...
	const isEditDisabled = isEditable || !RM.helper().isAuthorized(RM.commonConfig.arnConstants.PROGRAM_EDIT);
	...
	const actions = [
		{
			...
			variant: 'contained',
			hidden: !isEditable,
			onClick: async () => navigate('/program'),
		},
		{
			label: 'save',
			variant: 'outlined',
	...
	const breadcrumbs = [
		...
		{
			title: isCreateNewRecord ? 'Create Program' : values?.name || '',
		},
	];
	...
	return (
		<FormProvider {...formMethods}>
			<form onSubmit={formMethods.handleSubmit(onSubmit)}>
				<Page title="Program" breadcrumbs={breadcrumbs} actions={actions}>
					<Grid container spacing={2}>
						...
						<Grid size={6}>
							{isEditable ? (
								<RHAsyncAutoComplete
									name="department"
									label="Department"
									apiUrl={`${RM.commonConfig.apiBaseUrl}/department`}
									apiMethod="get"
									getOptionLabel={(option) => option?.name || ''}
								/>
							) : (
								<Label label="Department" value={values?.department?.name ?? '-'} />
							)}{' '}
						</Grid>
						<Grid size={6}>
							{isEditable ? (
						...
						<Grid size={6}>
							{isEditable ? (
								<RHTextField name="duration" label="Duration" fullWidth />
							) : (
								<Label label="Duration" value={values?.duration ?? '-'} />
							)}
						</Grid>
					</Grid>
				</Page>

...
// breadcrums
export const breadcrumbs = [{ title: 'Quota' }];
export const columns = [
	{ field: 'name', headerName: 'Title', width: 200 },
	{ field: 'description', headerName: 'Description', width: 200 },
	{
		field: 'isGovernmentQuota',
		headerName: 'Government Quota',
		width: 200,
...
export const customDataSource = {
	getRows: async (params) => {
		try {
			const response = await HTTP.get('/quota');
...
export const validationSchema = yup.object().shape({
	...
	isGovernmentQuota: yup.boolean().required('Government quota selection is required'),
});
	...
	try {
		if (isCreateNewRecord) {
			await createNewRecord({
				...
				isGovernmentQuota: values.isGovernmentQuota,
				code: RM.uuid(),
			});
			return;
		}
...
export const quotaOptions = [true, false];

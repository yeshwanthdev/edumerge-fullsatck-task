...
import { getSingleRecord, handleSubmit, validationSchema } from '@pages/Department/api';
...
const ObjectPage = (props) => {
	...
	const isEditDisabled = isEditable || !RM.helper().isAuthorized(RM.commonConfig.arnConstants.DEPARTMENT_EDIT);
	...
	const actions = [
		{
			...
			variant: 'contained',
			hidden: !isEditable,
			onClick: async () => navigate('/department'),
		},
		{
			label: 'save',
			variant: 'outlined',
	...
	const breadcrumbs = [
		...
		{
			title: isCreateNewRecord ? 'Create Department' : values?.name || '',
		},
	];
